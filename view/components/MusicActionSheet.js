...
const Music_ActionSheet = ({
  ...
  const handleUploadMusic = async () => {
    ...
    try {
      ...
      console.log('전달1:', uri);
      console.log('전달2: ', copyfilePath);

      MusicData(name, uri, copyfilePath);
          ...
          <Box
            ...
            <Box width={'230px'} height={'auto'}>
              <Text style={{fontSize: 14, fontWeight: 500, color: '#616161'}}>
                {fileFullName.length > 40
                  ? '"' + fileFullName.substring(0, 30) + '...' + '"\n'
                  : '"' + fileFullName + '"\n'}
              </Text>

              <Text style={{fontSize: 12, fontWeight: 500, color: '#ADADAD'}}>
                {audioSize2MB}MB
              </Text>
            </Box>
          </Box>
      ...
      <VStack
        ...
        <VStack width={'92%'} marginTop={8}>
          <Button
            size={'lg'}
            bg={'#2785F4'}
            onPress={handleConfirmInitialMusic}>
            <Text fontWeight={800} fontSize={'18px'} color={'white'}>
              초기화 하기
            </Text>
          </Button>
