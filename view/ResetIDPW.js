        ...
        <VStack flex={1} bg={'white'} p={5} justifyContent={'space-between'}>
          <VStack space={5}>
            <FormControl isRequired>
              <FormControl.Label _text={{bold: true, color: 'black'}}>
                이름
              </FormControl.Label>
              <Input
            ...
            renderItem={({item}) => (
              <Pressable onPress={() => handleItemPress(item)}>
                <HStack
                  ...
                  borderWidth={1}
                  borderColor={
                    selectedItem === item.username ? '#2785F4' : 'white'
                  }
                  alignItems={'center'}>
                  <VStack>
                    <Text>
                  ...
                  <Box
                    display={selectedItem === item.username ? 'block' : 'none'}>
                    <Ionicons
...
export const ResetIDPW = ({navigation, route}) => {
  ...
  const [index, setIndex] = React.useState(
    route.params.whichroute === 'one' ? 0 : 1,
  );
